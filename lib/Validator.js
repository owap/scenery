 ...
 *      + err - boolean indicating if there was an error validating the template
 *      + message - the result of the validation from aws-cli
 **/
Validator.prototype.validate = function(validationCallback){

    validationCallback = validationCallback || function(){ return true; };
    if(!this.templateFile){
        return validationCallback(true, 'No template file specified for validation');
    }
    ...
    async.waterfall([
        ...
        if(err){
            return validationCallback(true, err);
        }
        else if(!err && !message){
            return validationCallback(true,
